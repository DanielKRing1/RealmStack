...
export class _StackRealmCache {
    ...
    async createStack ({ dynamicRealmPath = DEFAULT_REALM_PATH_DYNAMIC_REALM, stackRealmPath = DEFAULT_REALM_PATH_STACK, stackName, snapshotProperties }: CreateStackParams): Promise<Realm> {
        ...
        // 5. Add to StackCache
        this.addStackRealm(stackName, newRealm);
    ...
    async loadStacks(stackRealmPath: string) {
        ...
        // 2. Track stacks
        const newRealm: Realm = await DynamicRealm.loadRealm(stackRealmPath);
        for(let stackName of stackNames) this._addStack(stackName, newRealm.path);
    ...
    addStackRealm(stackName: string, realm: Realm) {
        ...
        this._addStack(stackName, realmPath);
        this._addRealm(realm);
    }

    _addStack(stackName: string, realmPath: string) {
        this._stackCache[stackName] = realmPath;
    }
    _addRealm(realm: Realm) {
        const realmPath: RealmPath = realm.path;
    ...
    getLoadedRealms(): Realm[] { return Object.values(this._realmCache); }
    getLoadedRealmPaths(): string[] { return Object.keys(this._realmCache); }
    getLoadedStackNames(): string[] { return Object.keys(this._stackCache); }
